...
public:
    public:
    /**
     * @brief Return instance of the EdgeImuAbstraction
     *
     * @return EdgeImuAbstraction&
     */
    static EdgeImuAbstraction &instance()
    {
        if(!_instance)
        {
            _instance = new EdgeImuAbstraction();
        }
        return *_instance;
    }
...
private:
    EdgeImuAbstraction();
    static EdgeImuAbstraction *_instance;
    bool          isInitialized_ {false};
    BmiVariant    imu_ {BmiVariant::IMU_INVALID};

}; // class EdgeImuAbstraction

#define IMU EdgeImuAbstraction::instance()
