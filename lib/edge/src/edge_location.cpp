...
EdgeLocation *EdgeLocation::_instance = nullptr;
...
{
    static_cast<EdgeGnssAbstraction *>((void *)context)->setRadiusThreshold(value);
...
static int get_radius_cb(double &value, const void *context)
{
    ...
    static_cast<EdgeGnssAbstraction *>((void *)context)->getRadiusThreshold(temp);
...
int EdgeLocation::enter_location_config_cb(bool write, const void *context)
{
    if(write)
    {
...
int EdgeLocation::exit_location_config_cb(bool write, int status, const void *context)
{
    if(write && !status)
    {
...
int EdgeLocation::get_loc_cb(JSONValue *root)
{
   triggerLocPub(Trigger::IMMEDIATE);
   return 0;
}

void EdgeLocation::init(unsigned int gnssRetries)
{
    static ConfigObject location_desc
    (
        "location",
        {
            ConfigFloat("radius", get_radius_cb, set_radius_cb, &EdgeGnssAbstraction::instance()).min(0.0).max(1000000.0),
        ...
        std::bind(&EdgeLocation::enter_location_config_cb, this, _1, _2),
        std::bind(&EdgeLocation::exit_location_config_cb, this, _1, _2, _3)
    );
    ...
    CloudService::instance().registerCommand("get_loc", std::bind(&EdgeLocation::get_loc_cb, this, std::placeholders::_1));

    _last_location_publish_sec = System.uptime() - _config_state.interval_min_seconds;

    _sleep.registerSleepPrepare([this](EdgeSleepContext context){ this->onSleepPrepare(context); });
    _sleep.registerSleep([this](EdgeSleepContext context){ this->onSleep(context); });
    _sleep.registerSleepCancel([this](EdgeSleepContext context){ this->onSleepCancel(context); });
    _sleep.registerWake([this](EdgeSleepContext context){ this->onWake(context); });
    _sleep.registerStateChange([this](EdgeSleepContext context){ this->onSleepState(context); });

    _geofence.RegisterGeofenceCallback([this](CallbackContext& context){ this->onGeofenceCallback(context); });
    _geofence.init();

    CloudService::instance().registerCommand("loc-enhanced", std::bind(&EdgeLocation::enhanced_cb, this, std::placeholders::_1));

    _gnssRetryDefault = gnssRetries;
    setGnssCycle();
}

int EdgeLocation::buildEnhLocation(JSONValue& node, LocationPoint& point) {
...
int EdgeLocation::enhanced_cb(JSONValue *root) {
    LocationPoint point = {};
...
int EdgeLocation::triggerLocPub(Trigger type, const char *s)
{
...
void EdgeLocation::issue_location_publish_callbacks(CloudServiceStatus status, const String& req_event)
{
...
int EdgeLocation::location_publish_cb(CloudServiceStatus status, String&& req_event, std::uint32_t last_publish_time)
{
    ...
    {
        Log.info("location cb publish %lu unexpected status: %d", last_publish_time, (int)status);
    }
...
void EdgeLocation::location_publish()
{
    ...
    CloudService::instance().send(WITH_ACK,
        cloud_flags,
        std::bind(&EdgeLocation::location_publish_cb, this, std::placeholders::_1, std::placeholders::_2, _last_location_publish_sec));
}

void EdgeLocation::enableNetwork() {
    _sleep.forceFullWakeCycle();
}

int EdgeLocation::enableGnss() {
    auto ret = EdgeGnssAbstraction::instance().start();
...
int EdgeLocation::disableGnss() {
    return EdgeGnssAbstraction::instance().stop();
}

bool EdgeLocation::isSleepEnabled() {
    return !_sleep.isSleepDisabled();
}

EvaluationResults EdgeLocation::evaluatePublish(bool error) {
...
void EdgeLocation::onSleepPrepare(EdgeSleepContext context) {
    ...
    EdgeSleepError wakeRet = _sleep.wakeAtSeconds(wake);

    if (wakeRet == EdgeSleepError::TIME_IN_PAST) {
    ...
    Log.trace("EdgeLocation: last=%lu, interval=%ld, wake=%u", _last_location_publish_sec, interval, wake);
}

// The purpose of this callback is to alert us that sleep has been cancelled by another task or improper wake settings.
void EdgeLocation::onSleepCancel(EdgeSleepContext context) {
...
void EdgeLocation::onSleep(EdgeSleepContext context) {
    disableGnss();
}
...
void EdgeLocation::onWake(EdgeSleepContext context) {
...
void EdgeLocation::onSleepState(EdgeSleepContext context) {
    switch (context.reason) {
        case EdgeSleepReason::STATE_TO_CONNECTING: {
            break;
        }

        case EdgeSleepReason::STATE_TO_SHUTDOWN: {
...
void EdgeLocation::onGeofenceCallback(CallbackContext& context) {
...
size_t EdgeLocation::buildTowerInfo(JSONBufferWriter& writer, size_t size) {
    if (!_config_state_loop_safe.tower) {
        return 0;
    }

    EdgeCellular::instance().startScan();
    delay(EDGE_CELLULAR_SCAN_DELAY);
    ...
    EdgeCellular::instance().getServingTower(servingTower);
    if (servingTower.rat != RadioAccessTechnology::NONE) {
        ...
        EdgeCellular::instance().getNeighborTowers(towerList);
        auto towerCount = EdgeLocationMaxTowerSend - 1;  // one has already been taken as the serving tower
...
void EdgeLocation::wifi_cb(WiFiAccessPoint* wap, EdgeLocation* context) {
    if (context->wpsList.size() < EdgeLocationMaxWpsCollect)
        context->wpsList.append(*wap);
}

size_t EdgeLocation::buildWpsInfo(JSONBufferWriter& writer, size_t size) {
...
GnssState EdgeLocation::loopLocation(LocationPoint& cur_loc) {
    ...
    EdgeGnssAbstraction::instance().getStatus(locStatus);

    do {
        if (locStatus.error || (EdgeGnssAbstraction::instance().getLocation(cur_loc) != SYSTEM_ERROR_NONE)) {
        ...
        float radius {0.0};
        EdgeGnssAbstraction::instance().getRadiusThreshold(radius);
        if (radius) {
            bool outside {false};
            EdgeGnssAbstraction::instance().isOutsideRadius(outside, cur_loc);
            if (outside) {
...
void EdgeLocation::buildPublish(LocationPoint& cur_loc, bool error) {
    bool locked = (_config_state.gnss) ? cur_loc.locked : false;

    if(locked) {
        EdgeGnssAbstraction::instance().setWayPoint(cur_loc.latitude, cur_loc.longitude);
    }
...
void EdgeLocation::loop() {
    ...
    if (_config_state_loop_safe.gnss && _sleep.isFullWakeCycle() && _geofence.AnyGeofenceEnabled() && EdgeGnssAbstraction::instance().isLockStable()) {
    ...
    if(publishNow && (Particle.connected() ||
            EdgeLocationPublish::instance().isStoreEnabled()))
    {
