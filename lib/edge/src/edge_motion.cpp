...
EdgeMotion *EdgeMotion::_instance = nullptr;
...
int EdgeMotion::start(size_t eventDepth) {
    ...
    // Start the main EdgeMotion thread
    ret = os_thread_create(&thread_, "MOTSERV", OS_THREAD_PRIORITY_DEFAULT, EdgeMotion::thread, this, OS_THREAD_STACK_SIZE_DEFAULT);
    if (ret) {
...
int EdgeMotion::enableMotionDetection(MotionDetectionMode mode) {
...
int EdgeMotion::disableMotionDetection() {
    return enableMotionDetection(MotionDetectionMode::NONE);
}

MotionDetectionMode EdgeMotion::getMotionDetection() {
    return mode_;
}

int EdgeMotion::enableHighGDetection() {
    if (!isAnyAwake()) {
        CHECK(IMU.wakeup());
    }
...
int EdgeMotion::disableHighGDetection() {
...
HighGDetectionMode EdgeMotion::getHighGDetection() {
    return highGMode_;
}

int EdgeMotion::waitOnEvent(MotionEvent& event, system_tick_t timeout) {
...
void EdgeMotion::getStatistics(MotionCounters& stats) {
    stats = counters_;
}

// TODO: Migrate to conditional variables when they become available from the device OS
void EdgeMotion::thread(void* context) {
    EdgeMotion* self = static_cast<EdgeMotion*>(context);
    ...
    while (!exitLoop) {
        BmiEventType event;
        IMU.waitOnEvent(event, EdgeMotion::MOTION_TIMEOUT_DEFAULT);
        switch (event) {
...
size_t EdgeMotion::getQueueDepth() {
    return eventDepth_;
}

void EdgeMotion::setAwakeFlag(uint32_t bits) {
    awakeFlags_ |= bits;
}

void EdgeMotion::clearAwakeFlag(uint32_t bits) {
    awakeFlags_ &= ~bits;
}

bool EdgeMotion::isAnyAwake() {
