...
EdgeShipping *EdgeShipping::_instance = nullptr;

int EdgeShipping::regShutdownBeginCallback(ShippingModeCb begin)
{
...
int EdgeShipping::regShutdownIoCallback(ShippingModeCb io)
{
    _ioCallback = io;
...
int EdgeShipping::regShutdownFinalCallback(ShippingModeCb final)
{
...
void EdgeShipping::pmicHandler()
{
    EdgeShipping::instance()._pmicFire = true;
}

void EdgeShipping::shutdown()
{
    if(_ioCallback)
    {
        (void)_ioCallback();
    ...
    auto shipping = &EdgeShipping::instance();
    if (shipping->_checkPower)
    {
        ...
        // shipping mode.
        attachInterrupt(PMIC_INT, &EdgeShipping::pmicHandler, FALLING);
    }
...
int EdgeShipping::enter(bool checkPower)
{
    if(_beginCallback)
    {
    ...
    auto deferredShutdown = new Timer(SHIPPING_MODE_DEFER_DURATION_MS, &EdgeShipping::shutdown, *this, true);
    deferredShutdown->start();

    return 0; // compiler warnings about no return...
}

int EdgeShipping::enter_cb(JSONValue *root)
{
    return enter();
}

void EdgeShipping::init()
{
    CloudService::instance().registerCommand("enter_shipping", std::bind(&EdgeShipping::enter_cb, this, std::placeholders::_1));
}
