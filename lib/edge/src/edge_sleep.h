 ...
 *
 */
enum class EdgeSleepMode {
  Disable,                        /**< Sleep is disabled */
  Enable,                         /**< Sleep is enabled */
};

// Default configurations for Sleep
constexpr EdgeSleepMode EdgeSleepDefaultMode = EdgeSleepMode::Disable;
constexpr int32_t EdgeSleepDefaultExeMinTime = 10; // seconds
constexpr int32_t EdgeSleepDefaultConnMaxTime = 90; // seconds
constexpr int32_t EdgeSleepDefaultMaxTime = 86400; // seconds
constexpr system_tick_t EdgeSleepGracefulTimeout = 5 * 1000; // milliseconds
constexpr system_tick_t EdgeSleepShutdownTimeout = 4 * 1000; // milliseconds
constexpr system_tick_t EdgeSleepResetTimeout = 5 * 1000; // milliseconds
constexpr unsigned int EdgeSleepResetTimerDelay = 5 * 1000; // milliseconds

struct edge_sleep_config_t {
    EdgeSleepMode mode;
...
/**
 * @brief Return structure for Edge sleep functions.
 *
 */
struct EdgeSleepResult {
  SystemSleepResult result;       /**< System sleep result */
  EdgeSleepError error;        /**< Edge sleep internal error result */
};
...
struct EdgeSleepContext {
  EdgeSleepReason reason;      /**< Enumerated reason for the call */
  SystemSleepResult result;       /**< System sleep result */
...
/**
 * @brief EdgeSleep class to configure and manage sleep.
 *
 */
class EdgeSleep {
public:
  /**
   * @brief Singleton class instance access for EdgeSleep.
   *
   * @return EdgeSleep&
   */
  static EdgeSleep& instance() {
    if (!_instance) {
      _instance = new EdgeSleep();
    }
    return *_instance;
  }

  /**
   * @brief Initialize EdgeSleep.
   *
   ...
   *
   * @return EdgeSleepMode Enumeration for sleep configuration mode
   */
  EdgeSleepMode getConfigMode() {
  ...
  bool isSleepDisabled() {
    return (_config_state.mode == EdgeSleepMode::Disable);
  }
   ...
   * @retval EdgeSleepError::NONE Time was scheduled
   * @retval EdgeSleepError::TIME_IN_PAST Given time happened in the past
   * @retval EdgeSleepError::TIME_SKIPPED Given time happens later than a sooner wake request
   */
  ...
  /**
   * @brief Instruct the EdgeSleep class to power on the cellular modem and GNSS in short wake cycles.
   *
   * @retval SYSTEM_ERROR_NONE
   */
  ...
  /**
   * @brief Instruct EdgeSleep to power down the system after the current execution cycle.
   *
   */
  void forceShutdown() {
    _fullWakeupOverride = true;
  ...
  /**
   * @brief Indicate whether EdgeSleep is preparing for a pending shutdown request.
   *
  ...
  /**
   * @brief Instruct EdgeSleep to publish a vitals message before going to sleep.
   *
   */
  ...
  /**
   * @brief Main execution loop for the EdgeSleep class.  This must be executed within every system loop.
   *
   * @retval SYSTEM_ERROR_NONE
   */
  int loop();
...
private:
  /**
   * @brief Construct a new EdgeSleep singleton object
   *
   */
  EdgeSleep() :
    _onNetwork(false),
    ...
    _pendingReset(false),
    _executionState(EdgeExecutionState::BOOT),
...

    {
      ...
      _config_state = {
          .mode                     = EdgeSleepDefaultMode,
          .execute_min_seconds      = EdgeSleepDefaultExeMinTime,
          .connecting_max_seconds   = EdgeSleepDefaultConnMaxTime,
      };
    }

  /**
   * @brief Instruct EdgeSleep class that a publish occured.
   *
   */
  void annoucePublish() {
    _publishFlag = true;
  ...
  /**
   * @brief Instruct EdgeSleep to reset.
   *
   * @retval SYSTEM_ERROR_NONE
   */
  int enterReset();
   ...
   * @retval EdgeSleepError::TIME_SKIPPED Given time happens later than a sooner wake request
   */
  EdgeSleepError updateNextWake(uint64_t milliseconds);

  /**
   * @brief Prepare for sleep and wakeup.
   *
   * @return EdgeSleepResult
   */
  EdgeSleepResult sleep();
  ...
  // Cloud configuration for EdgeSleep
  edge_sleep_config_t _config_state;
  ...
  bool _pendingReset;
  EdgeExecutionState _executionState;
...
using TrackerSleepMode = EdgeSleepMode;
using tracker_sleep_config_t = edge_sleep_config_t;
using TrackerSleepError = EdgeSleepError;
using TrackerSleepReason = EdgeSleepReason;
using TrackerSleepResult = EdgeSleepResult;
using TrackerSleepContext = EdgeSleepContext;
using TrackerExecutionState = EdgeExecutionState;
using TrackerSleep = EdgeSleep;
