...
EdgeCellular *EdgeCellular::_instance = nullptr;

EdgeCellular::EdgeCellular() : _signal_update(0), _thread(nullptr)
{
    os_queue_create(&_commandQueue, sizeof(EdgeCellularCommand), 1, nullptr);
    _thread = new Thread("edge_cellular", [this]() {EdgeCellular::thread_f();}, OS_THREAD_PRIORITY_DEFAULT);
}

int EdgeCellular::startScan() {
    auto event = EdgeCellularCommand::Measure;
    CHECK_FALSE(os_queue_put(_commandQueue, &event, 0, nullptr), SYSTEM_ERROR_BUSY);

    return SYSTEM_ERROR_NONE;
}

int EdgeCellular::parseServeCell(const char* in, CellularServing& out) {
    CellularServing ret;
    char state[16] = {};
    char rat[16] = {};
...
int EdgeCellular::serving_cb(int type, const char* buf, int len, EdgeCellular* context) {
    if (type == TYPE_OK) {
        return RESP_OK;
    }
...
int EdgeCellular::parseCell(const char* in, CellularNeighbor& out) {
...
int EdgeCellular::neighbor_cb(int type, const char* buf, int len, EdgeCellular* context) {
    if (type == TYPE_OK) {
        return RESP_OK;
    }
...
void EdgeCellular::resetNeighborList() {
    _towerListSize = 0;
}

int EdgeCellular::addNeighborList(const CellularNeighbor& neighbor) {
    if (0 > _towerListSize) {
        resetNeighborList();
    }
...
EdgeCellularCommand EdgeCellular::waitOnEvent(system_tick_t timeout) {
    EdgeCellularCommand event {EdgeCellularCommand::None};
    auto ret = os_queue_take(_commandQueue, &event, timeout, nullptr);
    if (ret) {
        event = EdgeCellularCommand::None;
    }
...
void EdgeCellular::thread_f()
{
    auto loop = true;
    while (loop) {
        // Look for requests and provide a loop delay
        auto event = waitOnEvent(EDGE_CELLULAR_PERIOD_SUCCESS_MS);
        ...
        switch (event) {
            case EdgeCellularCommand::None:
                // Do nothing
                break;

            case EdgeCellularCommand::Exit:
                // Get out of main loop and join
                loop = false;
                break;

            case EdgeCellularCommand::Measure: {
...
int EdgeCellular::getSignal(CellularSignal &signal, unsigned int max_age)
{
...
unsigned int EdgeCellular::getSignalUpdate()
{
    return _signal_update;
}

int EdgeCellular::getServingTower(CellularServing& serving) {
    WITH_LOCK(mutex) {
...
int EdgeCellular::getNeighborTowers(Vector<CellularNeighbor>& neigbors) {
    WITH_LOCK(mutex) {
